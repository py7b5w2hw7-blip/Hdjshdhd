...
# УРОВНИ РЕФЕРАЛОВ (по количеству приглашённых) + награды (макс 50⭐)
LEVELS = [
    (0,    "🥉 Новичок",     15, 0),
    (5,    "🥈 Продвинутый", 20, 10),
    (15,   "🥇 Опытный",     25, 25),
...
def db_add_user(user: types.User, ref_by: int = None):
    ...
    cur = con.cursor()
    cur.execute("""
        INSERT OR IGNORE INTO users (user_id, username, first_name, joined_at, ref_by, banned, total_spent, last_daily, level_rewards)
        VALUES (?, ?, ?, ?, ?, 0, 0, NULL, '[]')
...
def db_add_purchase(user_id: int, plan: str, stars: int) -> tuple:
    ...
    db_update_balance(user_id, -stars)
...
async def show_profile(event):
    ...
    user_id = event.from_user.id if is_callback else event.from_user.id
...
async def daily_bonus(event):
    ...
    if db_can_claim_daily(user_id):
        ...
        if is_callback:
            await event.message.edit_text(text, parse_mode="HTML", reply_markup=kb_back_admin())
            await event.answer()
        else:
...
# ═══════════════════════════════════════════════════════
# ADMIN CALLBACKS (сокращённо, но все функции есть)
# ═══════════════════════════════════════════════════════
...
async def send_invoice(call: CallbackQuery):
    ...
    if balance < plan["stars"]:
        await call.answer(f"❌ Недостаточно средств! Нужно {plan['stars']}⭐, у вас {balance}⭐", show_alert=True)
        return
        ...
        if reward:
            try:
                await bot.send_message(
                    ...
                    f"💰 Награда: <b>+{reward}⭐</b> на баланс.\n"
